/**
 * UnequipItemHandler Integration Tests
 */
...
describe('UnequipItemHandler Integration', () => {
	let busManager: BusManager;
	let publishedEvents: DomainEvent[];

	beforeEach(() => {
		({ busManager, publishedEvents } = setupIntegrationTest({
			eventTypes: ['ItemUnequipped', 'CommandFailed']
		}));
	});

	describe('UnequipItem command', () => {
		it('should unequip item from adventurer', async () => {
			const adventurer = createTestAdventurer({ id: 'adv-1', state: 'Idle' });
			const item = createTestItem('item-1', 'Equipped');
			...
			const command = createTestCommand('UnequipItem', {
				itemId: 'item-1',
				adventurerId: 'adv-1',
				slot: 'weapon'
			});
